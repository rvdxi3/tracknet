            ...
            <div class="dashboard-header p-4 rounded-4" style="background: linear-gradient(135deg, #0f172a 0%, #1e3a8a 60%, #2563eb 100%); margin-top: -1rem; position: relative; overflow: hidden;">
                ...
                <div class="position-relative" style="z-index: 1;">
                    ...
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                        <div>
                            <h2 class="fw-bold text-white mb-1" style="font-size: 2rem;">Department Management</h2>
                            <p class="text-white-50 mb-0" style="font-size: 0.9rem;">Organise teams and assign users to departments</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0" style="border-radius: 20px; box-shadow: 0 10px 40px -5px rgba(13, 20, 40, 0.15); overflow: hidden;">
                ...
                <div class="card-header bg-transparent py-3 px-4 d-flex justify-content-between align-items-center" style="border-bottom: 1px solid #eef2f6;">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h6 class="fw-bold mb-0" style="color: #0f172a; font-size: 1rem;">All Departments</h6>
                            <small class="text-muted">Complete list of organisational departments</small>
                        </div>
                    </div>
                    <button type="button" onclick="openModal('addDeptOverlay')" class="btn btn-sm" style="background: linear-gradient(135deg, #2563eb, #1e3a8a); color: white; border: none; padding: 0.5rem 1.2rem; border-radius: 10px; font-weight: 600; font-size: 0.85rem;">
                        ...
                        <table class="table table-hover mb-0" style="min-width: 600px;">
                            <thead style="background: #f8fafd;">
                                <tr>
                                    ...
                                    <th class="py-3" style="color: #64748b; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em;">Description</th>
                                    <th class="py-3" style="color: #64748b; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em;">Members</th>
                            ...
                            <tbody>
                                ...
                                <tr style="border-bottom: 1px solid #eef2f6;">
                                    <td class="px-4 py-3" style="color: #94a3b8; font-size: 0.8rem; font-weight: 600;">{{ $department->id }}</td>
                                    <td class="py-3">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <span style="color: #0f172a; font-weight: 600;">{{ $department->name }}</span>
                                        </div>
                                    </td>
                                    <td class="py-3" style="color: #475569; font-size: 0.88rem;">
                                        {{ $department->description ? Str::limit($department->description, 55) : '—' }}
                                    </td>
                                    <td class="py-3">
                                        <span style="background: #2563eb20; color: #2563eb; padding: 0.3rem 0.75rem; border-radius: 30px; font-size: 0.75rem; font-weight: 600; display: inline-flex; align-items: center; gap: 0.3rem;">
                                            <i class="fas fa-users" style="font-size: 0.65rem;"></i> {{ $department->users_count }}
                                        </span>
                                    </td>
                                    <td class="py-3">
                                        <div class="d-flex align-items-center gap-1">
                                            {{-- View --}}
                                            <button type="button" onclick="openModal('viewDeptOverlay-{{ $department->id }}')"
                                            ...
                                            {{-- Edit --}}
                                            <button type="button" onclick="openModal('editDeptOverlay-{{ $department->id }}')"
                ...
                <div class="card-footer bg-transparent py-3 px-4" style="border-top: 1px solid #eef2f6;">
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                        <small class="text-muted">Showing {{ $departments->firstItem() }}–{{ $departments->lastItem() }} of {{ $departments->total() }} departments</small>
                        ...
                        <nav class="custom-pagination">
                            ...
                            @if($start > 1)
                                <a href="{{ $departments->url(1) }}" class="page-btn">1</a>
                            ...
                            @for($p = $start; $p <= $end; $p++)
                                ...
                                @else
                                    <a href="{{ $departments->url($p) }}" class="page-btn">{{ $p }}</a>
                                @endif
                            @endfor

                            @if($end < $last)
                                @if($end < $last - 1)<span class="page-btn dots">…</span>@endif
                                <a href="{{ $departments->url($last) }}" class="page-btn">{{ $last }}</a>
                            @endif
...
<div class="modal-overlay" id="addDeptOverlay" onclick="handleOverlayClick(event, 'addDeptOverlay')">
    <div class="modal-box" style="max-width: 520px;">
        <div class="modal-box-header">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="fw-bold mb-0" style="color: #0f172a;">Add New Department</h5>
                    <small class="text-muted">Create a new organisational department</small>
                </div>
            </div>
            <button class="modal-close-btn" onclick="closeModal('addDeptOverlay')"><i class="fas fa-times"></i></button>
        </div>
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <input type="text" name="name" class="modal-input @error('name') is-invalid @enderror"
                               placeholder="e.g. Human Resources" value="{{ old('name') }}" required>
                        @error('name')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-12">
                        <label class="modal-label">Description <span style="color:#94a3b8; font-weight:400; text-transform:none;">(optional)</span></label>
                        <textarea name="description" class="modal-input modal-textarea @error('description') is-invalid @enderror"
                                  placeholder="Briefly describe this department's purpose…">{{ old('description') }}</textarea>
                        @error('description')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                    </div>
                </div>
                <div class="modal-box-footer">
                    <button type="button" onclick="closeModal('addDeptOverlay')" class="modal-btn-cancel"><i class="fas fa-times me-2"></i>Cancel</button>
                    <button type="submit" class="modal-btn-submit"><i class="fas fa-building me-2"></i>Create Department</button>
                </div>
            </form>
        </div>
    </div>
...
{{-- ==================== PER-DEPARTMENT VIEW & EDIT MODALS ==================== --}}
@foreach($departments as $department)

{{-- VIEW MODAL --}}
<div class="modal-overlay" id="viewDeptOverlay-{{ $department->id }}" onclick="handleOverlayClick(event, 'viewDeptOverlay-{{ $department->id }}')">
    <div class="modal-box" style="max-width: 520px;">
        <div class="modal-box-header">
            <div class="d-flex align-items-center gap-3">
                <div class="modal-icon-box" style="background: linear-gradient(135deg, #2563eb, #1e3a8a);">
                ...
                <div>
                    <h5 class="fw-bold mb-0" style="color: #0f172a;">Department Details</h5>
                    <small class="text-muted">Overview of department information</small>
                </div>
            </div>
            <button class="modal-close-btn" onclick="closeModal('viewDeptOverlay-{{ $department->id }}')"><i class="fas fa-times"></i></button>
        </div>
        <div class="modal-box-body">
            ...
            <div class="text-center mb-4">
                ...
                <h6 class="fw-bold mb-1" style="color: #0f172a; font-size: 1.05rem;">{{ $department->name }}</h6>
                <span style="background: #2563eb20; color: #2563eb; padding: 0.3rem 0.9rem; border-radius: 30px; font-size: 0.75rem; font-weight: 600; display: inline-flex; align-items: center; gap: 0.3rem;">
                    <i class="fas fa-users" style="font-size: 0.65rem;"></i> {{ $department->users_count }} {{ Str::plural('Member', $department->users_count) }}
                </span>
            </div>
            ...
            <div style="background: #f8fafd; border-radius: 12px; padding: 1rem; display: flex; flex-direction: column; gap: 0.75rem; margin-bottom: 1.25rem;">
                <div class="d-flex align-items-start justify-content-between gap-3">
                    <span style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; flex-shrink: 0;">Description</span>
                    <span style="color: #0f172a; font-weight: 500; font-size: 0.88rem; text-align: right;">
                        {{ $department->description ?? '—' }}
                    </span>
                </div>
                <div style="height: 1px; background: #eef2f6;"></div>
                <div class="d-flex align-items-center justify-content-between">
                    <span style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em;">Created</span>
                    <span style="color: #0f172a; font-weight: 500; font-size: 0.88rem;">{{ $department->created_at->format('M d, Y') }}</span>
                </div>
                <div style="height: 1px; background: #eef2f6;"></div>
                <div class="d-flex align-items-center justify-content-between">
                    <span style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em;">Department ID</span>
                    <span style="color: #94a3b8; font-weight: 600; font-size: 0.85rem;">#{{ $department->id }}</span>
                </div>
            </div>
            ...
            <div>
                <p style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.6rem;">
                    Members Preview
                </p>
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    @foreach($department->users as $member)
                    @php
                        $roleColors = ['admin'=>['bg'=>'#ef444420','text'=>'#dc2626'],'inventory'=>['bg'=>'#2563eb20','text'=>'#2563eb'],'sales'=>['bg'=>'#05966920','text'=>'#059669']];
                        $mc = $roleColors[$member->role] ?? ['bg'=>'#64748b20','text'=>'#475569'];
                    @endphp
                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 0.75rem; background: #f8fafd; border-radius: 10px; border: 1px solid #eef2f6;">
                        <div style="display: flex; align-items: center; gap: 0.6rem;">
                            <div style="width: 30px; height: 30px; border-radius: 8px; background: linear-gradient(135deg, #2563eb, #1e3a8a); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 0.7rem; flex-shrink: 0;">
                                {{ strtoupper(substr($member->name, 0, 1)) }}
                            </div>
                            <div>
                                <div style="font-size: 0.85rem; font-weight: 600; color: #0f172a;">{{ $member->name }}</div>
                                <div style="font-size: 0.75rem; color: #94a3b8;">{{ $member->email }}</div>
                            </div>
                        </div>
                        <span style="background: {{ $mc['bg'] }}; color: {{ $mc['text'] }}; padding: 0.2rem 0.6rem; border-radius: 20px; font-size: 0.7rem; font-weight: 600;">
                            {{ ucfirst($member->role) }}
                        </span>
                    </div>
                    @endforeach
                </div>
                @if($department->users_count > 5)
                <p style="font-size: 0.78rem; color: #94a3b8; text-align: center; margin-top: 0.6rem;">
                    +{{ $department->users_count - 5 }} more member(s)
                </p>
                @endif
            </div>
            @else
            <div style="text-align: center; padding: 1rem; background: #f8fafd; border-radius: 12px; color: #94a3b8; font-size: 0.85rem;">
                <i class="fas fa-users mb-2 d-block" style="font-size: 1.5rem; opacity: 0.4;"></i>
                No members assigned yet
            </div>
            @endif

            <div class="modal-box-footer">
                <button type="button" onclick="closeModal('viewDeptOverlay-{{ $department->id }}')" class="modal-btn-cancel"><i class="fas fa-times me-2"></i>Close</button>
                <button type="button" onclick="closeModal('viewDeptOverlay-{{ $department->id }}'); openModal('editDeptOverlay-{{ $department->id }}')" class="modal-btn-submit">
...
{{-- EDIT MODAL --}}
<div class="modal-overlay" id="editDeptOverlay-{{ $department->id }}" onclick="handleOverlayClick(event, 'editDeptOverlay-{{ $department->id }}')">
    <div class="modal-box" style="max-width: 520px;">
        <div class="modal-box-header">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="fw-bold mb-0" style="color: #0f172a;">Edit Department</h5>
                    <small class="text-muted">Updating: {{ $department->name }}</small>
                </div>
            </div>
            <button class="modal-close-btn" onclick="closeModal('editDeptOverlay-{{ $department->id }}')"><i class="fas fa-times"></i></button>
        </div>
        <div class="modal-box-body">
            <form method="POST" action="{{ route('admin.departments.update', $department) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="_dept_id" value="{{ $department->id }}">

                <div class="row g-3">
                    <div class="col-12">
                        <label class="modal-label">Department Name</label>
                        <input type="text" name="name" class="modal-input @if($errors->any() && old('_dept_id') == $department->id) @error('name') is-invalid @enderror @endif"
                               value="{{ old('_dept_id') == $department->id ? old('name') : $department->name }}" required>
                        @if($errors->any() && old('_dept_id') == $department->id)
                            @error('name')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                        @endif
                    </div>
                    <div class="col-12">
                        <label class="modal-label">Description <span style="color:#94a3b8; font-weight:400; text-transform:none;">(optional)</span></label>
                        <textarea name="description" class="modal-input modal-textarea @if($errors->any() && old('_dept_id') == $department->id) @error('description') is-invalid @enderror @endif"
                                  placeholder="Briefly describe this department's purpose…">{{ old('_dept_id') == $department->id ? old('description') : $department->description }}</textarea>
                        @if($errors->any() && old('_dept_id') == $department->id)
                            @error('description')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                        @endif
                    </div>
                ...
                <div class="modal-box-footer">
                    <button type="button" onclick="closeModal('editDeptOverlay-{{ $department->id }}')" class="modal-btn-cancel"><i class="fas fa-times me-2"></i>Cancel</button>
...
document.addEventListener('DOMContentLoaded', function() {
    @if($errors->any())
        @if(old('_dept_id'))
            openModal('editDeptOverlay-{{ old('_dept_id') }}');
        @else
